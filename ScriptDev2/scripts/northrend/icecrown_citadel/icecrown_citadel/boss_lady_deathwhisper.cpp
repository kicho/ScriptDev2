...
struct MANGOS_DLL_DECL boss_deathwhisperAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (Phase == 2)
        {
            ...
            {
                if (Unit* pTarget = m_creature->SelectRandomUnfriendlyTarget())
                     DoCastSpellIfCan(pTarget, m_bIsRegularMode ? SPELL_FROSTBOLT_VOLLEY : H_SPELL_FROSTBOLT_VOLLEY);
                FrostboltVolley_Timer = 40000;
            }
            ...
            {
                if (Unit* pTarget = m_creature->SelectRandomUnfriendlyTarget())
                     DoCastSpellIfCan(pTarget, m_bIsRegularMode ? SPELL_FROSTBOLT : H_SPELL_FROSTBOLT);
                Frostbolt_Timer = 15000;
            }
        ...
        {
            if (Unit* pTarget = m_creature->SelectRandomUnfriendlyTarget())
                 DoCastSpellIfCan(pTarget, m_bIsRegularMode ? SPELL_DEATH_AND_DECAY : H_SPELL_DEATH_AND_DECAY);
            DeathAndDecay_Timer = 30000;
        }
